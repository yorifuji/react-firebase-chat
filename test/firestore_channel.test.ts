...
beforeEach( async () => {
  testEnv = await testing.initializeTestEnvironment({
    projectId: "my-project-id-channel",
    firestore: {
...
describe("更新", () => {
  ...
  test("成功", async () => {
    const firestore = testEnv.authenticatedContext("alice").firestore()
    await testing.assertSucceeds(firestore.doc("channels/channel1").update({
      name: "channel2",
  ...
  test("失敗（未認証）", async () => {
    const firestore = testEnv.unauthenticatedContext().firestore()
    await testing.assertFails(firestore.doc("channels/channel1").update({
      name: "channel2",
      updatedAt: serverTimestamp()
    }))
  })
