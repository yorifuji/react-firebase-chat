...
describe("作成", () => {
  test("成功", async () => {
    ...
    await testing.assertSucceeds(firestore.doc("/channels/channel/posts/post").set({
      owner: "alice",
      from: "alice",
      body: "message0",
  ...
  test("失敗（未認証）", async () => {
    ...
    await testing.assertFails(firestore.doc("/channels/channel/posts/post").set({
      owner: "alice",
      from: "alice",
      body: "message0",
      createdAt: serverTimestamp(),
...
describe("更新", () => {
  ...
  test("成功", async () => {
    const firestore = testEnv.authenticatedContext("alice").firestore()
    await testing.assertSucceeds(firestore.doc("/channels/channel/posts/post").update({
      owner: "alice",
      from: "alice",
      body: "message1",
  ...
  test("失敗（未認証）", async () => {
    const firestore = testEnv.unauthenticatedContext().firestore()
    await testing.assertFails(firestore.doc("/channels/channel/posts/post").update({
      owner: "alice",
      from: "alice",
      body: "message1",
      updatedAt: serverTimestamp(),
