...
describe("作成", () => {
  ...
  test("成功", async () => {
    const firestore = testEnv.authenticatedContext("alice").firestore()
    await testing.assertSucceeds(firestore.collection("/channels/channel/posts/post/reactions").add({
      uid: "alice",
      channel: "channel",
      post: "postID",
      emoji: "emoji1",
  ...
  test("失敗（未認証）", async () => {
    ...
    await testing.assertFails(firestore.collection("/channels/channel/posts/post/reactions").add({
      uid: "alice",
      channel: "channel",
      post: "postID",
      emoji: "emoji1",
